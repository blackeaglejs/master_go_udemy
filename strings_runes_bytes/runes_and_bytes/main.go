package main

import (
	"fmt"
	"strings"
	"unicode/utf8"
)

func main() {
	// Anything can be encoded in a string.
	// Runes are the golang name for a larger comp sci thing called a code point.
	// Runes are defined with single-quoted strings, and are int32 code points.
	var1 := 'a'
	fmt.Printf("Type: %T, Value:%d \n", var1, var1) // => Type: int32, Value:97
	// value is 97 (the code point for a)

	// Rune is also an alias for int32, and when checking the type of a rune object, it will return int32.
	// Go uses more bytes than strings for each rune beause they are non Ascii characters.
	// declaring a string value that contains non-ascii characters
	str := "ţară" // ţară means country in Romanian
	// 't', 'a' ,'r' and 'a' are runes and each rune occupies beetween 1 and 4 bytes.

	//The len() built-in function returns the no. of bytes not runes or chars.
	fmt.Println(len(str)) // -> 6,  4 runes in the string but the length is 6

	// returning the number of runes in the string
	m := utf8.RuneCountInString(str)
	fmt.Println(m) // => 4

	// You can loop over a string with a for loop, and use %c to format the individual byte type.
	// You can find the Decoded rune and the size using the utf8.DecodeRuneInString function.
	// UTF8 package is really useful.
	// decoding a string byte by byte
	for i := 0; i < len(str); i++ {
		fmt.Printf("%c", str[i]) // -> Å£arÄ
	}

	fmt.Println("\n" + strings.Repeat("#", 10))

	// decoding a string rune by rune manually:
	for i := 0; i < len(str); {
		r, size := utf8.DecodeRuneInString(str[i:]) // it returns the rune in string in variable r
		//and its size in bytes in variable size

		// printing out each rune
		fmt.Printf("%c", r) // -> ţară
		i += size           // incrementing i by the size of the rune in bytes
	}

	fmt.Println("\n" + strings.Repeat("#", 10))

	// For decoding, we want to use a range loop.
	for i, r := range str { //the first value returned by range is the index of the byte in string where rune starts
		fmt.Printf("%d -> %c", i, r) // => ţară
	}

	// You can get the length of a rune using the len function. If you use a rune character, you will find that
	// it will return different lengths for non-Ascii characters than ASCII characters.
}
